...
make_table.select_one <- function(srvyr.design.grouped, entry) {
  ...
  if(entry$omit_na & length(disagg_vars) >1){
    res.wide <- res.wide %>% 
      drop_na(entry$disaggregate.variables)
  } else if (!entry$omit_na & length(disagg_vars) >1){
    res.wide <- res.wide %>% 
      mutate(!!sym(entry$disaggregate.variables) := replace_na(!!sym(entry$disaggregate.variables) %>% as.character, "NA"))
  }
  return(res.wide)
}

make_table.select_multiple <- function(srvyr.design.grouped, tool.choices=NULL,label_colname = NULL, entry){
...
make_table <- function(srvyr.design,tool.choices = NULL,label_colname = NULL, entry, disagg.var){
  ...
  res <- switch (entry$func,
                 ...
                 select_one = { srvyr.design.grouped  %>%
                     make_table.select_one(entry) },
                 select_multiple = { srvyr.design.grouped %>%
                     make_table.select_multiple(entry,tool.choices= tool.choices,label_colname = label_colname) }
  )
